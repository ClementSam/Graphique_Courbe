...
class GraphService:
    ...
    def _generate_unique_graph_name(self):
        logger.debug("🔄 [GraphService._generate_unique_graph_name] Recherche d'un nom unique...")
        index = 1
        while f"Graphique {index}" in self.state.graphs:
            logger.debug(f"🔁 [GraphService._generate_unique_graph_name] Graphique {index} existe déjà")
            index += 1
        result = f"Graphique {index}"
        logger.debug(f"🎯 [GraphService._generate_unique_graph_name] Nom unique trouvé : {result}")
        return result
    ...
    def select_graph(self, name: str):
        ...
        if name not in self.state.graphs:
            logger.warning(f"❌ [GraphService.select_graph] Graphique '{name}' introuvable dans AppState.")
        ...
        logger.debug(f"📢 [GraphService.select_graph] Emission du signal graph_selected")
    ...
    def rename_curve(self, old_name: str, new_name: str):
        ...
        if not graph:
            logger.warning("⚠️ [GraphService.rename_curve] Aucun graphique sélectionné !")
            raise ValueError("Aucun graphique sélectionné")

        if any(c.name == new_name for c in graph.curves):
            logger.warning(f"❌ [GraphService.rename_curve] Une courbe nommée '{new_name}' existe déjà.")
        ...
        logger.warning(f"❌ [GraphService.rename_curve] Courbe '{old_name}' non trouvée")
    ...
    def add_graph(self, name: str = None):
        ...
        if name in self.state.graphs:
            logger.warning(f"❌ [GraphService.add_graph] Le graphique '{name}' existe déjà !")
        ...
        logger.debug(f"📢 [GraphService.add_graph] Emission du signal graph_selected pour '{name}'")
        signal_bus.graph_selected.emit(name)
    
        logger.debug(f"📢 [GraphService.add_graph] Emission du signal graph_updated")
    ...
    def add_curve(self, graph_name: str, curve: CurveData = None):
        ...
        if not graph:
            logger.warning(f"❌ [GraphService.add_curve] Graphique '{graph_name}' introuvable dans AppState")
            return
        ...
        logger.debug(f"📢 [GraphService.add_curve] Emission du signal curve_selected pour '{curve.name}' dans '{graph.name}'")
        signal_bus.curve_selected.emit(graph.name, curve.name)
    
        logger.debug(f"📢 [GraphService.add_curve] Emission des signaux curve_list_updated et curve_updated")
    ...
    def remove_graph(self, name: str):
        ...
        if name not in self.state.graphs:
            logger.warning(f"❌ [GraphService.remove_graph] Graphique '{name}' introuvable")
        ...
        logger.debug(f"📢 [GraphService.remove_graph] Emission du signal graph_updated")
        signal_bus.graph_updated.emit()
    ...
    def remove_curve(self, curve_name: str):
        graph = self.state.current_graph
        if not graph:
            logger.warning("❌ [GraphService.remove_curve] Aucun graphique sélectionné.")
            return
        ...
        if not curve_to_remove:
            logger.warning(f"❌ [GraphService.remove_curve] Courbe '{curve_name}' introuvable.")
            return
        ...
        logger.debug(f"📢 [GraphService.remove_curve] Emission du signal curve_updated")
        signal_bus.curve_updated.emit()
    ...
    def bring_curve_to_front(self):
        ...
        if not graph or not curve:
            logger.warning("⚠️ [GraphService.bring_curve_to_front] Aucune courbe ou graphique sélectionné")
            return
