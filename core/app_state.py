...
class AppState:
    ...
    def add_graph(self, name: str):
        ...
        if name in self.graphs:
            logger.debug(f"⛔️ [AppState.add_graph] Graphique '{name}' déjà présent !")
    ...
    def remove_graph(self, name: str):
        logger.debug(f"❌ [AppState.remove_graph] Suppression du graphique '{name}'")
        if name not in self.graphs:
            raise KeyError(f"Le graphique '{name}' n'existe pas.")
        del self.graphs[name]
        if self.current_graph and self.current_graph.name == name:
            logger.debug(f"⚠️ [AppState.remove_graph] Graphique supprimé était actif, nettoyage de la sélection.")
    ...
    def select_curve(self, curve_name: str):
        ...
        if not self.current_graph:
            logger.debug("⚠️ [AppState.select_curve] Aucun graphique sélectionné")
            self.current_curve = None
            return
